
/* writes 2's complement value to the 16 bit dac 
 usefule numbers:
 0V = -32768   // 65536 range corresponds to the full range of the stage (220 um)
 5V = 0
 10V = 32767 
 250nm step is 75 units (74.473)
 */

void setupDac(){
  /*
 the 16bit dac chip bus 
 D0-D7       ->  PA0-PA7
 D8-D15      ->  PC7-PC0
 
 the 16bit dac chip logic controls
 A0n         ->  PG1
 A1n         ->  PG0
 WRn         ->  PD7
 CLRn        ->  PG2
 */
  DDRA = B11111111;
  DDRC = B11111111;
  DDRG = B00000111;
  DDRD = B10000000;
  PORTG = B00000100;
  PORTD = B00000000;
  write2Dac(0); 
}
void write2Dac(int value){
  byte upperHalf = reverse_byte((byte) (value >> 8));
  byte lowerHalf = (byte) value;
  PORTC = upperHalf;
  PORTA = lowerHalf;
}

// 16-bit DAC outputs are converted to 8-bit outputs of Arduino
static const unsigned char reverse_byte_Table[] = {
  ,
};

unsigned char reverse_byte(unsigned char x){
  return reverse_byte_Table[x];
}


